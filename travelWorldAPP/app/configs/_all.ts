
/// <reference path="../../typings/angularjs/angular.d.ts" />
/// <reference path="../../typings/angularjs/angular-route.d.ts" />
/// <reference path="../../typings/angular-ui-bootstrap/angular-ui-bootstrap.d.ts" />
/// <reference path="../../typings/ui-grid/ui-grid.d.ts" />
/// <reference path="../../typings/toastr/toastr.d.ts" />

/// <reference path="AppConfig.ts" />
/// <reference path="Modules.ts" />
/// <reference path="Routes.ts" />
/// <reference path="Startup.ts" />

/// <reference path="../contracts/IRootScope.ts" />
/// <reference path="../contracts/IHttpService.ts" />
/// <reference path="../contracts/ILocationService.ts" />
/// <reference path="../contracts/IModalService.ts" />
/// <reference path="../contracts/IModalServiceInstance.ts" />
/// <reference path="../contracts/IRouteParamsService.ts" />
/// <reference path="../contracts/ITimeoutService.ts" />


/// <reference path="../models/api-models/SosUsuario.ts" />
/// <reference path="../models/api-models/SistemaContexto.ts" />
/// <reference path="../models/api-models/Post.ts" />
/// <reference path="../models/api-models/Category.ts" />
/// <reference path="../models/api-models/Continent.ts" />
/// <reference path="../models/api-models/Country.ts" />
/// <reference path="../models/api-models/City.ts" />

/// <reference path="../models/app-models/LoginModel.ts" />


/// <reference path="../services/app-service-base/IAppService.ts" />
/// <reference path="../services/app-service-base/AppServiceBase.ts" />
/// <reference path="../services/CategoryService.ts" />
/// <reference path="../services/ContinentService.ts" />
/// <reference path="../services/CountryService.ts" />
/// <reference path="../services/CityService.ts" />
/// <reference path="../services/AutenticacaoService.ts" />


// Controller

/// <reference path="../controllers/autenticacao/LoginController.ts" />
/// <reference path="../controllers/autenticacao/LogoutController.ts" />
/// <reference path="../controllers/shareds/SharedController.ts" /> 

/// <reference path="../controllers/category/CategoryController.ts" />
/// <reference path="../controllers/category/ModalCreateController.ts" />
/// <reference path="../controllers/category/ModalEditController.ts" />
/// <reference path="../controllers/category/ModalDeleteController.ts" />

/// <reference path="../controllers/continent/ContinentController.ts" />
/// <reference path="../controllers/continent/ModalCreateContinentController.ts" />
/// <reference path="../controllers/continent/ModalEditContinentController.ts" />
/// <reference path="../controllers/continent/ModalDeleteContinentController.ts" />

/// <reference path="../controllers/country/CountryController.ts" />
/// <reference path="../controllers/country/ModalCreateCountryController.ts" />
/// <reference path="../controllers/country/ModalEditCountryController.ts" />
/// <reference path="../controllers/country/ModalDeleteCountryController.ts" />

/// <reference path="../controllers/city/CityController.ts" />
/// <reference path="../controllers/city/ModalCreateCityController.ts" />
/// <reference path="../controllers/city/ModalEditCityController.ts" />
/// <reference path="../controllers/city/ModalDeleteCityController.ts" />

/// <reference path="../controllers/menu/SideBarController.ts" />
/// <reference path="../controllers/HomeController.ts" />







